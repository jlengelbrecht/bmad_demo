...
| **Progress data model** | Single table:<br>`progress(kind TEXT, id TEXT, completed_at TEXT NULL, PRIMARY KEY(kind, id))`<br>Where `kind ∈ {'lesson', 'lab', 'capstone-session', 'capstone-step', 'capstone-tool', 'capstone-target', 'capstone-tool-session', 'capstone-session-lock'}`.<br><br>**Encoding conventions:**<br>• Lessons: `kind='lesson'`, `id='lesson-1'` … `id='lesson-6'`. `completed_at` set on completion.<br>• Labs: `kind='lab'`, `id='solo'` / `'sync'` / `'async-story-review'`. `completed_at` set on completion.<br>• **Capstone session:** `kind='capstone-session'`, `id=<compact UTC timestamp>` (e.g. `'20260507T143022Z'`). **`completed_at IS NULL`** while in progress; **clean ISO** when Phase 9 completes; **`'aborted-<ISO>'` sentinel** when the Story 6.5 abort flow tears the session down. Three states → three semantics, all on one column.<br>• Capstone phase: `kind='capstone-step'`, `id='<session-id>/<phase-name>'` (e.g., `'20260507T143022Z/brief'`, `'…/prd'`, `'…/architecture'`, `'…/epics-and-stories'`, `'…/adr'`, `'…/dev-story-1.1'`). Each phase's `completed_at` is set when the phase-done gate (FR-3.21) is satisfied.<br>• **Capstone tool selection:** `kind='capstone-tool'`, `id='<session-id>'`. The chosen tool id (`claude-code` / `codex` / `github-copilot`) is stored in `completed_at` as the value (overloading: this column historically stores ISO timestamps, but for `capstone-tool` rows it stores the literal tool id). *Rationale:* avoids adding a new column and keeps the single-table-one-idiom property for v1; the tool id is short, the row is keyed by session, and there's no risk of confusion since reads always filter by `kind`. v1.1 may introduce a typed extras column if a third metadata kind appears.<br>• **Capstone target dir:** `kind='capstone-target'`, `id='<session-id>'`. The trainee-chosen absolute path is stored in `completed_at` similarly.<br>• **Capstone tool-native session id:** `kind='capstone-tool-session'`, `id='<sessionId>/<phase>'`. `completed_at` overloaded to store the tool-native session id captured from the first SSE `system/init` event for that phase, so subsequent turns spawn with `--resume <session-id>`. Per Story 5.7 AC10.<br>• **Capstone two-tab session lock:** `kind='capstone-session-lock'`, `id='<sessionId>'`. `completed_at` overloaded to store an ISO expiry timestamp for the two-tab lock; rows expire on TTL (60s) or are explicitly invalidated on take-over. Per Story 6.1 AC6.<br><br>**CHECK constraint on `completed_at`:** `(completed_at IS NULL OR completed_at LIKE '____-__-__T__:__:__%Z' OR completed_at LIKE 'aborted-%')`. The `aborted-` prefix accommodates the abort-sentinel pattern. The CHECK is positional smoke, not full ISO validation; the producer is `new Date().toISOString()` so the CHECK is belt-and-suspenders. Story 6.5 AC4 owns the relaxation. | — | Smallest model satisfying FR-2.1 / 2.2 / 2.3 / 2.4 / 2.5 (resume + multi-session + tool/target persistence) **and** FR-3.x. Three additional kinds + one sentinel pattern were introduced by Stories 5.7 / 6.1 / 6.5 to support per-phase tool-native session resume, two-tab session locking, and abort-vs-complete distinguishability without adding new columns or tables. The column-overload is a localized v1 affordance flagged in this rationale rather than spreading through the codebase as silent magic. |
...
The three optional `RunOptions` fields beyond the base `cmd/args/cwd/env/signal` set were added in Stories 5.1 / 5.7 / 6.5 to support per-session subprocess.log routing, abort-by-metadata lookups, and stdin-write capture for spawn-per-message chat consumers.
