...
describe("lesson sequence", () => {
  it("returns seven lessons sorted by leading number prefix (numeric, not lexicographic)", () => {
    const sequence = getLessonSequence();
    expect(sequence).toHaveLength(7);
    expect(sequence.map((l) => l.number)).toEqual([1, 2, 3, 4, 5, 6, 7]);
    expect(sequence[0].slug).toBe("1-what-is-bmad");
    expect(sequence[5].slug).toBe("6-the-bmad-ecosystem-and-installer");
    expect(sequence[6].slug).toBe("7-from-lessons-to-capstone");
  });
...
describe("getNeighbors", () => {
  ...
  it("returns no next for the last lesson", () => {
    const { prev, next } = getNeighbors("7-from-lessons-to-capstone");
    expect(prev?.number).toBe(6);
