...
export function buildBootstrapExplainer(
  ...
  return [
    ...
    `- ${skillsLine}`,
    `- **\`_bmad-output/planning-artifacts/\`** and **\`_bmad-output/implementation-artifacts/\`** — empty for now. The next phases populate them: brief → PRD → architecture → epics + stories → working code for story 1.1. Files in these folders are the *contract* between phases.`,
